...
class NoviceSystem {
  ...
  private:
	...
	DirectX::XMFLOAT4 FloatColor(unsigned int color);
...
void NoviceSystem::CreateConstBuffer() {
	...
	//平行投影による射影行列の生成
	DirectX::XMMATRIX mat = DirectX::XMMatrixOrthographicOffCenterLH(
...
DirectX::XMFLOAT4 NoviceSystem::FloatColor(unsigned int color) {
...
void NoviceSystem::DrawBox(int x, int y, int w, int h, float angle, unsigned int color) {
	...
	DirectX::XMFLOAT4 colorf = FloatColor(color);
...
void NoviceSystem::DrawQuad(
  ...
  int srcH, int textureHandle, unsigned int color) {
	...
	// 平行投影による射影行列の設定
	constBufferForQuads_[indexQuad_].address->mat = DirectX::XMMatrixOrthographicOffCenterLH(
	  0, float(dxCommon_->GetBackBufferWidth()), float(dxCommon_->GetBackBufferHeight()), 0, 0, 1);
	// 頂点バッファの設定
...
const DirectX::XMFLOAT2& NoviceSystem::GetMousePosition() { return input_->GetMousePosition(); }
...
int Novice::GetMousePosition(int* positionX, int* positionY) {
	// ClientRect内の座標
	const DirectX::XMFLOAT2& mousePosition = sNoviceSystem->GetMousePosition();
